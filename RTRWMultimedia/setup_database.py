...
with open("SetupDB.cs", "w", encoding="utf-8") as f:
    f.write(cs_code)

res = subprocess.run([r"C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe", "/out:SetupDB.exe", "SetupDB.cs"], capture_output=True, text=True)
print("Build SetupDB:", res.stdout, res.stderr)
if os.path.exists("SetupDB.exe"):
    res2 = subprocess.run(["SetupDB.exe"], capture_output=True, text=True)
    print("Execute SetupDB:", res2.stdout)
